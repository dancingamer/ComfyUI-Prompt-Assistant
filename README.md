...
> - Ollama 포트가 바뀌어도 실행 중인 서버를 찾아 씁니다. Windows가 11434/11435를 막아 다른 포트로 `ollama serve`를 켜도 Base URL을 다시 적을 필요가 없습니다.
> - 번역 버튼 관련해 수정한 곳은 `js/utils/promptFormatter.js`의 `detectLanguage` 하나뿐입니다. 백엔드는 원래부터 한국어를 지원해서 손대지 않았습니다.
> - 태그 도구의 태그 이름 1514개와 분류 79개를 `config/tags_template.json`에서 한국어로 옮겼습니다. 프롬프트에 실제로 들어가는 영어 태그 값은 그대로입니다.
>
> ### 받은 뒤 해야 할 설정
>
> 아래 세 가지는 ComfyUI 사용자 폴더에 저장되므로 git으로 따라오지 않습니다. 저장소를 받은 PC마다 한 번씩 맞춰야 합니다.
>
> **1. 화면을 한국어로.** `설정`(`Ctrl + ,`) → `✨프롬프트 도우미` → `인터페이스` → `표시 언어` → `한국어`. 고르면 페이지가 자동으로 새로고침되고 버튼, 설정 화면, 팝업, 오류 알림이 모두 한국어로 바뀝니다. 지정하지 않으면 기본값인 중국어로 표시됩니다.
>
> **2. 번역 서비스를 Ollama로.** 기본값이 Baidu라서 API 키를 넣지 않으면 번역 버튼이 `Baidu: 먼저 Baidu 번역 API의 APP_ID와 SECRET_KEY를 설정하세요` 오류만 냅니다. Baidu는 중국어 중심이라 한국어에도 맞지 않습니다.
>
> `설정` → `✨프롬프트 도우미` → `번역 설정` → `API 관리자`를 열어 쓸 모델을 추가합니다. 그다음 프롬프트 입력칸 위의 **번역 버튼을 우클릭**하면 서비스 목록이 뜨는데, 거기서 그 모델을 고르면 됩니다.
>
> Base URL은 기본값(`http://localhost:11434`) 그대로 둬도 됩니다. 저장된 주소가 죽어 있으면 실행 중인 Ollama 포트를 찾아 붙습니다. `base_url`에 `/v1`을 붙이면 OpenAI 호환 API로, 안 붙이면 네이티브 API로 호출됩니다.
>
> **3. 태그 도구의 태그 파일.** 한국어 태그 이름은 `config/tags_template.json`에 담겨 git으로 따라옵니다. 하지만 이 템플릿은 아래 폴더에 CSV가 **하나도 없을 때만** 읽힙니다.
>
> ```
> ComfyUI/user/default/prompt-assistant/tags/
> ```
>
> 그래서 **새로 설치하는 PC**는 첫 실행 때 템플릿에서 태그 파일이 만들어지므로 할 일이 없습니다. 반면 **이미 쓰고 있던 PC**는 그 폴더에 중국어 `默认标签.csv`가 남아 있어 템플릿을 다시 읽지 않습니다. 한국어로 바꿔 둔 PC의 `默认标签.csv`를 같은 폴더에 덮어쓰세요.
>
> ```
> ComfyUI/user/default/prompt-assistant/tags/默认标签.csv
> ```
>
> 파일 이름은 코드가 참조하므로 바꾸지 마세요. 또 `默认标签.csv`만 지우고 재시작해도, 같은 폴더에 `用户标签.csv`처럼 다른 CSV가 있으면 템플릿에서 다시 만들어지지 않습니다.
>
> 덮어쓴 뒤 브라우저를 새로고침하면 바뀐 이름이 보입니다. 직접 만든 태그 파일은 영향을 받지 않습니다. 즐겨찾기는 `{표시 이름: 태그 값}`으로 저장되므로, 중국어 이름으로 등록해 둔 즐겨찾기가 있으면 그 항목만 옛 이름으로 남습니다. 다시 등록하면 됩니다.
>
> 원본 업데이트를 가져오려면:
>
> ```bash
