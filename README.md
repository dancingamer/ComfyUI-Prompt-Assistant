...
>
> `설정` → `✨프롬프트 도우미` → `번역 설정` → `API 관리자`를 열어 Ollama의 `Base URL`이 실제 포트와 맞는지 확인하고 쓸 모델을 추가합니다. 그다음 프롬프트 입력칸 위의 **번역 버튼을 우클릭**하면 서비스 목록이 뜨는데, 거기서 그 모델을 고르면 됩니다.
>
> `base_url`에 `/v1`을 붙이면 OpenAI 호환 API로, 안 붙이면 네이티브 API로 호출됩니다.
>
